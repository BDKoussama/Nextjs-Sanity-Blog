...
export const getAuthorsPaths = async () => {
    const paths = await client.fetch(`*[_type == "author" && defined(slug.current)][].slug.current `)
    return paths;   
}

export const getCurrentAuthor = async (slug) => {

    const author = await client.fetch(groq`*[_type == "author" && slug.current == $slug][0]{
             name,
            "slug": slug.current,
            description,
            image,
